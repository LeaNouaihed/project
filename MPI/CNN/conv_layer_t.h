...
void activate()
{
    ...
    int filters_per_rank = (filters.size() + size - 1) / size;
    int filter_start = filters_per_rank * rank;
    int filter_end = filter_start + filters_per_rank;
    if (rank == size - 1) {
        filter_end = filters.size();
    }
    
    for (int filter = filter_start; filter < filter_end; filter++)
    {
    ...
    if (rank == 0)
    {
        ...
        {
            int filter_start = filters_per_rank * r;
            int filter_end = filter_start + filters_per_rank;
            if (r == size - 1) {
                filter_end = filters.size();
            }
            for (int filter = filter_start; filter < filter_end; filter++)
            {
                MPI_Status status;
                tensor_t<float>& filter_data = filters[filter];
                MPI_Recv(filter_data.data, filter_data.size.x * filter_data.size.y * filter_data.size.z, MPI_FLOAT, r, filter, MPI_COMM_WORLD, &status);
            }
        }
    }
    else
    {
        ...
        {
            tensor_t<float>& filter_data = filters[filter];
            MPI_Send(filter_data.data, filter_data.size.x * filter_data.size.y * filter_data.size.z, MPI_FLOAT, 0, filter, MPI_COMM_WORLD);
        }
    }
