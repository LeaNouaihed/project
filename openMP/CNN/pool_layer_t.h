...
struct pool_layer_t
{
	...
	void activate()
{
    // Parallelize the outer loop over output rows
            ...
            for (int z = 0; z < out.size.z; z++) {
                ...
                for (int i = 0; i < extend_filter; i++) {
                    for (int j = 0; j < extend_filter; j++) {
                        float v = in(mapped.x + i, mapped.y + j, z); // Fetch value from input
                        if (v > mval) {
                            mval = v; // Update max value
                        }
                    }
                }

                out(x, y, z) = mval; // Store the maximum value in the output tensor
            }
        }
    }
}
